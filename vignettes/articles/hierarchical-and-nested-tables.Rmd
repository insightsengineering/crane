...
The function reports on each row the total number of subjects with the specified AE within the current context determined by the variables specified. 

If multiple AEs are recorded for a single subject, all of these events are counted once for that subject. 
...
Instead of event *rates* per subject this function calculates overall event *counts*, with no option to specify an `id` parameter. 
...
To do so, the innermost hierarchy variable (the last variable in `variable`) must be converted to an ordered factor variable.
