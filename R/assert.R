...
check_list_of_variables <- function (x)
{
  ...
  res <- checkmate::check_list(x, names = "named", min.len = 1,
...
check_df_with_variables <- function (df, variables, na_level = NULL)
{
  ...
  if (isFALSE(err_flag)) {
    ...
    return(paste(deparse(substitute(df)), "does not contain all specified variables as column names. Missing from data frame:",
                 paste(vars, collapse = ", ")))
  }
  if (!is.null(na_level)) {
    checkmate::assert_string(na_level)
    res <- unlist(lapply(as.list(df)[unlist(variables)],
                         function(x) any(x == na_level)))
    if (any(res)) {
      return(paste0(deparse(substitute(df)), " contains explicit na_level (",
                                ...
                                any.missing = TRUE, n.levels = NULL, len = NULL)
{
  ...
  res <- checkmate::check_factor(x, min.levels = min.levels,
                                 ...
                                 add = NULL)
{
  if (missing(x))
    stop(sprintf("argument \"%s\" is missing, with no default",
                 .var.name))
  res = check_valid_factor(x, min.levels, max.levels, null.ok,
