...
#' @param col_counts (`integer` vector, optional)\cr
#'   A vector of column counts for each arm, used to display the number of subjects
#'   in each arm in the column headers. If not provided, column counts will not be displayed.
#'
...
afit <- tern.mmrm::fit_mmrm(
  vars = list(response = "FEV1", covariates = c("RACE", "SEX", "ARMCD"), id = "USUBJID",
              arm = "ARMCD", visit =
                "AVISIT"),
  mmrm::fev_data,
  conf_level = 0.95,
...
tbl_mmrm(tidy(afit), mmrm::fev_data |> dplyr::mutate(AVISIT = "BASE"), arm = "ARMCD", visit = "AVISIT", baseline_aval = "FEV1")

tbl_mmrm <- function(df_tidy_mmrm, df_baseline, arm, visit, baseline_aval, col_counts = NULL) {
  # arm <- "arm_label"
  # visit <- "AVISIT"
  # baseline_aval <- "AVAL"
  # df_baseline <- df_mmrmt01_base
  # df_tidy_mmrm <- df
...
.get_adj_mean_se <- function(data, ...) {
  val <- if (nrow(data) == 0 || is.na(data$estimate_est[1])) { "-" } else {
...
.get_adj_mean_ci <- function(data, ...) {
  val <- if (nrow(data) == 0 || is.na(data$lower_cl_est[1])) { "-" } else {
...
.get_diff_se <- function(data, ...) {
  val <- if (nrow(data) == 0 || is.na(data$estimate_contr[1])) { "-" } else {
...
.get_diff_ci <- function(data, ...) {
  val <- if (nrow(data) == 0 || is.na(data$lower_cl_contr[1])) { "-" } else {
...
.get_pval <- function(data, ...) {
  val <- if (nrow(data) == 0 || is.na(data$p_value[1])) { "-" } else {
