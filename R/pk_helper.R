...
#' observations and dosing timing. Two substitutions are made:
#'
#' * A statistic with no value is replaced by `"NE"` (not estimable). This
#'   covers variability statistics that need more than one observation, such as
#'   the standard deviation, CV%, and geometric CV% when `n = 1`. Counts (e.g.
#'   `"No. obs."`) are always available and never become `"NE"`.
#' * When too many observations are BLQ, statistics that cannot be meaningfully
#'   reported are replaced by `"ND"` (not determined).
#'
#' Because the `"NE"` substitution happens here, the formatters (`fmt_3sig()`,
#' `fmt_pct()`) stay purely numeric and return `NA` for missing values; the
#' `"NE"`/`"ND"` display logic is applied downstream by this function.
#'
#' @examples
#' pk_imputation_rules("1.23", "Mean", blq_ratio = 0.5, postdose = TRUE, rule = "1/3")
#' # SD is not estimable with a single observation
#' pk_imputation_rules(NA, "SD", blq_ratio = 0, postdose = TRUE)
#'
#' @export
pk_imputation_rules <- function(stat_val, label, blq_ratio, postdose, rule = "1/3") {
  ...
  # a statistic with no value is not estimable (e.g. SD/CV with n = 1); this is
  # independent of the BLQ rules. counts are always available, so keep them.
  count_labels <- c("No. obs.", "Number of LTR/BLQ")
  if ((is.na(stat_val) || identical(stat_val, "NA")) && !label %in% count_labels) {
    return("NE")
  }
